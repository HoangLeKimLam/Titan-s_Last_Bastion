...
# 15 -> 30: giảm ~1 nửa tốc độ loot (cùng số lính, factor nhỏ đi 1 nửa) —
# ảnh hưởng ĐỀU mọi node (6 node cố định + Item Cache), không đổi tỉ lệ
# tương đối giữa các node. KHÔNG đụng HAZARD_S_REF (dòng dưới, khác mục
# đích: xác suất gặp titan, trùng giá trị cũ chỉ là tình cờ).
SOLDIER_REF: float = 30.0
